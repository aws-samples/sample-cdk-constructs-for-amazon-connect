...
async function associateInstanceStorageConfig(
...
async function updateInstanceStorageConfig(
...
async function disassociateInstanceStorageConfig(
...
function convertCfnParamsToSdkParams(
  ...
  const instanceId = (props.instanceArn as string).split('/').pop(); // TODO: remove cast
  if (!instanceId) {
